...
def parse_mtx(folder: str) -> anndata.AnnData:
    ...
    adata = anndata.read_mtx(f"{folder}/matrix.mtx.gz").T
    adata.obs_names = pd.read_table(f"{folder}/barcodes.tsv.gz", header=None)[0]

    # switch gene ids for gene symbol
    genes = pd.read_table(f"{folder}/features.tsv.gz", header=None, index_col=0)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Generate count matrix for sample based on well and sample name"
    )
    parser.add_argument("--version", action="version", version="1.0")
    parser.add_argument("--pool", action="store", type=str, help="Pool name")
    parser.add_argument(
        "solo_out", action="store", type=str, help="Path to Solo.out folder"
    )
    ...
    parser.add_argument(
        ...
        action="store",
        type=str,
        help="Path to barcode set based on plate id",
    )
